...
void IP_LQG_PCI6221_2015a_output(void)
{
  ...
  for (iy = 0; iy < 5; iy++) {
    /* StateSpace: '<Root>/LQE' */
    IP_LQG_PCI6221_2015a_B.LQE[iy] = 0.0;
    for (ci = 0; ci < 5; ci++) {
      IP_LQG_PCI6221_2015a_B.LQE[iy] += IP_LQG_PCI6221_2015a_P.LQE_C[ci * 5 + iy]
        * IP_LQG_PCI6221_2015a_X.LQE_CSTATE[ci];
    }

    /* End of StateSpace: '<Root>/LQE' */

    /* Gain: '<Root>/Gain' */
    u0 += -IP_LQG_PCI6221_2015a_P.K[iy] * IP_LQG_PCI6221_2015a_B.LQE[iy];
  }

  /* Gain: '<Root>/Gain' */
  IP_LQG_PCI6221_2015a_B.Gain = u0;
  if (rtmIsMajorTimeStep(IP_LQG_PCI6221_2015a_M)) {
  ...
  if (IP_LQG_PCI6221_2015a_B.Fcn_n >=
    ...
    u0 = IP_LQG_PCI6221_2015a_B.Gain;
  } else {
   ...
   *  Saturate: '<S1>/Motor protection'
   */
  IP_LQG_PCI6221_2015a_B.Gain_p = IP_LQG_PCI6221_2015a_P.Gain_Gain * u0;
  if (rtmIsMajorTimeStep(IP_LQG_PCI6221_2015a_M)) {
    ...
    {
      {
        ...
        RTBIO_DriverIO(0, ANALOGOUTPUT, IOWRITE, 1,
                       ...
                       &IP_LQG_PCI6221_2015a_B.Gain_p, &parm);
      }
    }
  }
  ...
  /* SignalConversion: '<Root>/TmpSignal ConversionAtLQEInport1' */
  IP_LQG_PCI6221_2015a_B.TmpSignalConversionAtLQEInport1[0] =
    IP_LQG_PCI6221_2015a_B.Gain;
  IP_LQG_PCI6221_2015a_B.TmpSignalConversionAtLQEInport1[1] =
    IP_LQG_PCI6221_2015a_B.Sensorsswitch[0];
  IP_LQG_PCI6221_2015a_B.TmpSignalConversionAtLQEInport1[2] =
    IP_LQG_PCI6221_2015a_B.Sensorsswitch[1];
...
void IP_LQG_PCI6221_2015a_derivatives(void)
{
  ...
  /* Derivatives for StateSpace: '<Root>/LQE' */
  for (is = 0; is < 5; is++) {
    _rtXdot->LQE_CSTATE[is] = 0.0;
    for (ci = 0; ci < 5; ci++) {
      _rtXdot->LQE_CSTATE[is] += (IP_LQG_PCI6221_2015a_P.A[ci * 5 + is] -
        IP_LQG_PCI6221_2015a_ConstP.LQE_rtw_collapsed_sub_expr_0[ci * 5 + is]) *
        IP_LQG_PCI6221_2015a_X.LQE_CSTATE[ci];
    }

    _rtXdot->LQE_CSTATE[is] += IP_LQG_PCI6221_2015a_P.LQE_B[is] *
      IP_LQG_PCI6221_2015a_B.TmpSignalConversionAtLQEInport1[0];
    _rtXdot->LQE_CSTATE[is] += IP_LQG_PCI6221_2015a_P.LQE_B[5 + is] *
      IP_LQG_PCI6221_2015a_B.TmpSignalConversionAtLQEInport1[1];
    _rtXdot->LQE_CSTATE[is] += IP_LQG_PCI6221_2015a_P.LQE_B[10 + is] *
      IP_LQG_PCI6221_2015a_B.TmpSignalConversionAtLQEInport1[2];
  }

  /* End of Derivatives for StateSpace: '<Root>/LQE' */
}
...
void IP_LQG_PCI6221_2015a_initialize(void)
{
...

  {
    ...
    /* InitializeConditions for StateSpace: '<Root>/LQE' */
    for (is = 0; is < 5; is++) {
      IP_LQG_PCI6221_2015a_X.LQE_CSTATE[is] = IP_LQG_PCI6221_2015a_P.LQE_X0;
    }

    /* End of InitializeConditions for StateSpace: '<Root>/LQE' */
...
RT_MODEL_IP_LQG_PCI6221_2015a_T *IP_LQG_PCI6221_2015a(void)
{
  ...
  /* External mode info */
  IP_LQG_PCI6221_2015a_M->Sizes.checksums[0] = (2512821902U);
  IP_LQG_PCI6221_2015a_M->Sizes.checksums[1] = (3712649244U);
  IP_LQG_PCI6221_2015a_M->Sizes.checksums[2] = (299326120U);
  IP_LQG_PCI6221_2015a_M->Sizes.checksums[3] = (2770756081U);
  ...
  IP_LQG_PCI6221_2015a_M->Sizes.numBlocks = (44);/* Number of blocks */
  IP_LQG_PCI6221_2015a_M->Sizes.numBlockIO = (11);/* Number of block outputs */
  IP_LQG_PCI6221_2015a_M->Sizes.numBlockPrms = (104);/* Sum of parameter "widths" */
