...
void IP_LQG_PCI6221_2015a_output(void)
{
  ...
  /* MATLAB Function: '<Root>/Dead Zone Compensator' */
  /* MATLAB Function 'Dead Zone Compensator': '<S1>:1' */
  if (IP_LQG_PCI6221_2015a_B.LQG > 0.0) {
    /* '<S1>:1:2' */
    /* '<S1>:1:3' */
    rtb_u_out = IP_LQG_PCI6221_2015a_B.LQG + 0.22;
  } else if (IP_LQG_PCI6221_2015a_B.LQG < 0.0) {
    /* '<S1>:1:4' */
    /* '<S1>:1:5' */
    rtb_u_out = IP_LQG_PCI6221_2015a_B.LQG - 0.22;
  } else {
    /* '<S1>:1:7' */
    rtb_u_out = IP_LQG_PCI6221_2015a_B.LQG;
  }

  /* End of MATLAB Function: '<Root>/Dead Zone Compensator' */
  if (rtmIsMajorTimeStep(IP_LQG_PCI6221_2015a_M)) {
    ...
    /* DataTypeConversion: '<S2>/Boolean to double 1' incorporates:
    ...
    /* DataTypeConversion: '<S2>/Boolean to double 2' incorporates:
  ...
  if (IP_LQG_PCI6221_2015a_M->Timing.t[0] < IP_LQG_PCI6221_2015a_P.TIME_DELAY) {
    tmp = IP_LQG_PCI6221_2015a_P.Timedelay_Y0;
  } else {
    tmp = IP_LQG_PCI6221_2015a_P.Timedelay_YFinal;
  }

  /* End of Step: '<S2>/Time delay' */

  /* Fcn: '<S2>/Fcn' */
  IP_LQG_PCI6221_2015a_B.Fcn_n = ((real_T)(fabs
    (IP_LQG_PCI6221_2015a_B.Booleantodouble1) > 0.0) + (real_T)(fabs
    (IP_LQG_PCI6221_2015a_B.Booleantodouble2) > 0.0)) + (real_T)(fabs(tmp) > 0.0);

  /* Switch: '<S2>/Motor switch' incorporates:
   *  Constant: '<S2>/Const motor'
   */
  if (!(IP_LQG_PCI6221_2015a_B.Fcn_n >=
        IP_LQG_PCI6221_2015a_P.Motorswitch_Threshold)) {
    rtb_u_out = IP_LQG_PCI6221_2015a_P.Constmotor_Value;
  }

  /* End of Switch: '<S2>/Motor switch' */

  /* Saturate: '<S2>/Motor protection' */
  if (rtb_u_out > IP_LQG_PCI6221_2015a_P.V_MAX) {
    rtb_u_out = IP_LQG_PCI6221_2015a_P.V_MAX;
  } else {
    if (rtb_u_out < IP_LQG_PCI6221_2015a_P.V_MIN) {
      rtb_u_out = IP_LQG_PCI6221_2015a_P.V_MIN;
    }
  }
   ...
   *  Saturate: '<S2>/Motor protection'
   */
  IP_LQG_PCI6221_2015a_B.Gain = IP_LQG_PCI6221_2015a_P.Gain_Gain * rtb_u_out;
  if (rtmIsMajorTimeStep(IP_LQG_PCI6221_2015a_M)) {
  ...
  /* Switch: '<S2>/Sensors switch' incorporates:
   *  Constant: '<S2>/Const sensors'
   */
  if (IP_LQG_PCI6221_2015a_B.Fcn_n >=
  ...
  /* End of Switch: '<S2>/Sensors switch' */
...
void IP_LQG_PCI6221_2015a_derivatives(void)
{
  ...
  for (is = 0; is < 6; is++) {
    ...
    for (ci = 0; ci < 6; ci++) {
      _rtXdot->LQG_CSTATE[is] += ((IP_LQG_PCI6221_2015a_P.A_aug[ci * 6 + is] -
        IP_LQG_PCI6221_2015a_ConstP.LQG_rtw_collapsed_sub_expr_0[ci * 6 + is]) -
        IP_LQG_PCI6221_2015a_ConstP.LQG_rtw_collapsed_sub_expr_1[ci * 6 + is]) *
        IP_LQG_PCI6221_2015a_X.LQG_CSTATE[ci];
    }

    _rtXdot->LQG_CSTATE[is] += IP_LQG_PCI6221_2015a_P.L_aug[is] *
      IP_LQG_PCI6221_2015a_B.Sensorsswitch[0];
    _rtXdot->LQG_CSTATE[is] += IP_LQG_PCI6221_2015a_P.L_aug[6 + is] *
      IP_LQG_PCI6221_2015a_B.Sensorsswitch[1];
  }
...
void IP_LQG_PCI6221_2015a_initialize(void)
{
...

  {
    ...
    for (is = 0; is < 6; is++) {
      IP_LQG_PCI6221_2015a_X.LQG_CSTATE[is] = IP_LQG_PCI6221_2015a_P.LQG_X0[is];
    }
...
RT_MODEL_IP_LQG_PCI6221_2015a_T *IP_LQG_PCI6221_2015a(void)
{
  ...
  IP_LQG_PCI6221_2015a_M->ModelData.intgData.f[3] =
    IP_LQG_PCI6221_2015a_M->ModelData.odeF[3];
  IP_LQG_PCI6221_2015a_M->ModelData.intgData.f[4] =
    IP_LQG_PCI6221_2015a_M->ModelData.odeF[4];
  IP_LQG_PCI6221_2015a_M->ModelData.intgData.f[5] =
    IP_LQG_PCI6221_2015a_M->ModelData.odeF[5];
  ...
  rtsiSetSolverName(&IP_LQG_PCI6221_2015a_M->solverInfo,"ode5");
  ...
  rtmSetTFinal(IP_LQG_PCI6221_2015a_M, 30.0);
  IP_LQG_PCI6221_2015a_M->Timing.stepSize0 = 0.001;
  IP_LQG_PCI6221_2015a_M->Timing.stepSize1 = 0.001;

  /* External mode info */
  IP_LQG_PCI6221_2015a_M->Sizes.checksums[0] = (4221155365U);
  IP_LQG_PCI6221_2015a_M->Sizes.checksums[1] = (844877139U);
  IP_LQG_PCI6221_2015a_M->Sizes.checksums[2] = (3118515205U);
  IP_LQG_PCI6221_2015a_M->Sizes.checksums[3] = (1676923939U);
  ...
  /* Initialize Sizes */
  IP_LQG_PCI6221_2015a_M->Sizes.numContStates = (6);/* Number of continuous states */
  ...
  IP_LQG_PCI6221_2015a_M->Sizes.numBlocks = (43);/* Number of blocks */
  IP_LQG_PCI6221_2015a_M->Sizes.numBlockIO = (9);/* Number of block outputs */
  IP_LQG_PCI6221_2015a_M->Sizes.numBlockPrms = (93);/* Sum of parameter "widths" */
